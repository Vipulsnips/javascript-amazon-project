...
export function renderOrderSummary(){
  ...
  cart.forEach((cartItem) => {
    ...
    // SAFETY CHECK
    if (!matchingProduct) {
      console.error('Missing product for cart item:', cartItem);
      return;
    }
   ...
   const delivery=getDeliveryOption(cartItem.deliveryOptionId);
    ...
    // Fallback if delivery option missing
    if (!delivery) {
      delivery = deliveryOptions[0];
    }

    const deliveryDate = dayjs().add(delivery.deliverDays, 'days');
  ...
  function deliveryOptionsHTML(matchingProduct, cartItem) {
    ...
    deliveryOptions.forEach((option) => {
      const deliveryDate = dayjs().add(option.deliverDays, 'days');
      const dateString = deliveryDate.format('dddd, MMMM D');
  ...
  function updateCartQuantity() {
    document.querySelector('.js-quantity-checkout').innerHTML =
      calculateCartQuantity();
  }
